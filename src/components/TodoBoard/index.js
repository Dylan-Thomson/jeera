...
const initialLists = {};
initialLists[TaskStateEnum.TODO] = ["Come up with a better idea.", "Feed cats", "Think of todo items that aren't about food"];
  ...
  return (
    <section>
      {Object.keys(lists).map((key, i) => {
          return <TodoList key={key} todos={lists[key]} listType={key}/>
      })}
    </section>
  );
};
